...
class Quadtree {
    ...
    insert(entity) {
        if (this.level < maxLevel) {
            this.child[this.getIndex(entity.x, entity.y)].insert(entity);
        } else {
    ...
    retrieve(x, y, dist, dx, dy, range) {
        ...
        console.log("x:" + dx + ", y:" + dy + ", angle: " + angle);
        // var i = angle - range*.5;
        // var end = i + range;
        // for (i; i < end; i+= range/8) {

        var i = 0;
        for (i; i < Math.PI*2; i+= Math.PI/8) {
            quad = this.getQuad(x + Math.cos(i) * (dist-1), y + Math.sin(i) * (dist-1), dist); // TODO check for duplicates
            if (dist >= quad.height) {
                var children = quad.getChildren();
                var l = children.length;
                var j = 0;
        ...
        for (var a of nearObjs) {
            if (this.near(a, {"x":x, "y":y}, dist, dx, dy, range*.5)) {
                ret.push(a);
            }
        }
        return ret;
    ...
    near(other, me, d, dx, dy, range) {
        ...
        if (Math.min(Math.abs(a-a2), Math.PI*2-Math.abs(a-a2)) < range) {
